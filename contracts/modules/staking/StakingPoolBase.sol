...
contract StakingPoolBase is Module, IERC900, CapperRole  {
    ...
    mapping(address => uint256) public userCap; //Limit of staked tokens which can be minted for a user during deposit
    ...
    modifier isUserCapEnabledForStakeFor(uint256 stake) {
        if (stakingCapEnabled && !(vipUserEnabled && isVipUser[_msgSender()])) {
            require((stakingCap > totalStaked() && (stakingCap-totalStaked() >= stake)), "StakingModule: stake exceeds staking cap");
        }
        ...
        if(userCapEnabled) {
            ...
            require(cap >= stake, "StakingPoolBase: stake exceeds cap");
            cap = cap.sub(stake);
    ...
    function unstakeAllUnlocked(bytes memory _data) public returns(uint256) {
         ...
         for(uint256 i=personalStakeIndex; i<stakeHolders[_msgSender()].personalStakes.length; i++) {
             if (stakeHolders[_msgSender()].personalStakes[i].unlockedTimestamp <= block.timestamp) {
                uint256 stakeAmount = stakeHolders[_msgSender()].personalStakes[i].actualAmount;
                unstakeAllAmount = unstakeAllAmount+stakeAmount;
                withdrawStake(stakeAmount, _data);
             }
         }
